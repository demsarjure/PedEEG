...
%% load the data
raw = pop_fileio(strcat('D:/Work/EEG/', subject, '/', subject, '.vhdr'), 'dataformat', 'auto');
raw.setname = 'raw';

%% load electrode locations
raw = pop_editset(raw, 'run', [], 'chanlocs', 'D:/Work/EEG/64BPMR+ref.ced');
...
%% plot spectral
figure; pop_spectopo(rest_raw, 1, [0  300000], 'EEG' , 'percent', 50, 'freqrange', [2 25], 'electrodes', 'off');

%% remove bad electrodes
bad_electrodes = [7, 17, 32, 46];
...
%% plot spectral
figure; pop_spectopo(rest, 1, [0  300000], 'EEG' , 'percent', 50, 'freqrange', [2 25], 'electrodes', 'off');
...
%% save
pop_saveset(rest, 'filename', strcat(subject, '_rest_cleaned.set'), 'filepath', strcat('D:/Work/EEG/', subject, '/'));
